...
\alignauthor
Steve Chapel\\
\email{schapel@umich.edu}
\alignauthor
Rahul Jha\\
\email{rahuljha@umich.edu}
%% % 1st. author
%% \alignauthor
...
When spreadsheets were initially developed, computers had
low-resolution screens which could hold very little information and
display only text-based information. Today, although nearly every
computer has a large, high-resolution color graphical display, we are
stuck with the paradigm of spreadsheets as a huge array of cells in
which formulas are copied and modified. Formulas cannot be seen except
for a cell-at-a-time view. Cells are referred to with an arcane
letter-and-number syntax that belies the relative nature of the
relationship between cell names and their use. This paper explores
several ideas to form a new paradigm for spreadsheets for the purpose
of making them easier to use correctly.
...
\category{D.1.7}{Software}{Programming Techniques}[Visual Programming]
\category{H.4.1}{Information Systems Applications}{Office Automation}[Spreadsheets]

\terms{Reliability}

\keywords{visual programming, spreadsheets, units}

\section{Introduction}
Intro goes here.\cite{forms/3}
...
Currently, we do not have an implementation of units of measurement or
other type checking. We do have a syntax for annotating constants in
program text with units of measurement, e.g.
\verb#9.8# \verb#(newton-meter)#, \verb#15# \verb#(pounds/in^2)#,
\verb#440# \verb#second^-1#.

Should we consider zero to be a value of any unit, as in Kennedy's
system? It would make sense for adding, subtracting, or comparing
units u and v where 0 u = 0 v, but it would not work for degrees
Celsius and degrees Fahrenheit where 0 in one unit is not 0 in
another.

It may be necessary to allow the user to cast an expression to a
different unit of measurement at run time. It is unclear how this
would work in general, for example, if the user wished to write a
square root routine that could take different units of measurement as
its parameter. That is, if the user wished to compute
\verb#sqrt(4 meter^2)# and \verb#sqrt(9 foot^2)#, the units of the
result would need to be computed at run time.

We could allow the user to get the raw value of a type in
dimensionless units. We could also provide a way to get the units of a
value, and a way to construct a value with given units.

\verb#function abs(x) {# \\
\verb#    if (x >= 0*x) x else -x# \\
\verb#}#
 
\verb#function sqrt(x) {# \\
\verb#    r = halfunit(x);# \\
\verb#    while (abs(r*r-x) > x/1.0e9)# \\
\verb#        r = r - (r*r-x) / (2*r);# \\
\verb#    return r;# \\
\verb#}#

\verb#function halfunit(x) {# \\
\verb#    u = unit(x);    // [[meter,2]]# \\
\verb#    i = 0;# \\
\verb#    while (i < len(u)) {# \\
\verb#        u[i][1] = floor(u[i][1] / 2);# \\
\verb#        i = i + 1;# \\
\verb#    }# \\
\verb#    // u = [[meter,1]]# \\
\verb#    return one(u);    // 1 meter# \\
\verb#}#
